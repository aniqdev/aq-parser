...
if (isset($_POST['getjson'])) {
    ...
    foreach ($idsArr as $va) {
		...
		$game_id         = $reqs[$j-1]['id'];
		if (isset($arrItem[0])) {
			$item1_id    = mysql_escape_string(trim(strip_tags($arrItem[0][0])));
			$item1_name  = mysql_escape_string(trim(strip_tags($arrItem[0][1])));
			$item1_price = mysql_escape_string(trim(strip_tags($arrItem[0][2])));
			$item1_desc  = mysql_escape_string(trim(strip_tags($arrItem[0][3])));
		}
		if (isset($arrItem[1])) {
			$item2_id    = mysql_escape_string(trim(strip_tags($arrItem[1][0])));
		 	$item2_name  = mysql_escape_string(trim(strip_tags($arrItem[1][1])));
			$item2_price = mysql_escape_string(trim(strip_tags($arrItem[1][2])));
			$item2_desc  = mysql_escape_string(trim(strip_tags($arrItem[1][3])));
		}
		if (isset($arrItem[2])) {
			$item3_id    = mysql_escape_string(trim(strip_tags($arrItem[2][0])));
		 	$item3_name  = mysql_escape_string(trim(strip_tags($arrItem[2][1])));
			$item3_price = mysql_escape_string(trim(strip_tags($arrItem[2][2])));
			$item3_desc  = mysql_escape_string(trim(strip_tags($arrItem[2][3])));
		}
...
<button id="getjson">Спарсить</button>
