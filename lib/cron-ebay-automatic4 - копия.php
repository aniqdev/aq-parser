...
$dataex = arrayDB("SELECT value FROM aq_settings WHERE name='exrate'");
if($dataex) $dataex = $dataex[0]['value']; // 67
else{
...
// Входные данные:
	if (!$msg_email || stripos($msg_email, '{{PRODUCT}}') === false) {
		add_comment_to_order($gig_order_id, 'Please check out the original template!');
		continue;
	}
...
// 2.5 Исключение заказов без имейла.
...
// 2.6 Исключение пользователей с низким рейтингом(меньше 5) если цена больше 5 евро.
...
// 2.7 Исключение новых аккаунтов(меньше 30 дней) если цена больше 5 евро.
...
// 2.8 Исключить заказы если пользователь совершил больше 3х покупок за неделю.
// Входные данные:
...
// 2.9 Исключить заказы если немец, сумма заказов в течение недели >10€
// Входные данные:
...
// Входные данные: payed_price, chosen_item_price, dataex
		...
		// переделать логику. так как если сработает фильт 3,1 то сработает и этот
		// уберет товар с продажи и перезапишет комментарий хотя этого делать не нужно
		$is_removed = $ebay2Obj->removeFromSale($ebay_item_id);
		$rem = $is_removed ? 'has been' : 'was NOT';
