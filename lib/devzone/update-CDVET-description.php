...
if (isset($_POST['action']) && $_POST['action'] === 'iterate') {
	...
	$where = "WHERE ebay_id <> '' AND extra_field <> 'desc_upadated5'";
	...
	$extra_field_mark = 'desc_upadated_with_feed';
	...
	// 	$row = $cd_arr[$shop_id];
	// }
	if ($cdvet_feed[$shop_id]) {
		$row = [
			'I' => $cdvet_feed[$shop_id][9],
			'C' => $cdvet_feed[$shop_id][4],
		];
	}
	else{
		echo json_encode([
	...
	if (count($img_arr) < 1) {
		echo json_encode([
			'resp' => 'There are no chosen pictures!',
