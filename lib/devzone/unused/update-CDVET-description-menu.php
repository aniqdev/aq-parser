...
if (isset($_POST['action']) && $_POST['action'] === 'iterate') {
	...
	$extra_field_mark = 'desc_upadated6';
	...
	$description = str_replace('href="http://stores.ebay.de/cdvet/_i.html?_fsub',
							 'href="//stores.ebay.de/cdvet?cat',
							  $description);
...
function send_post(offset) {
	$.post('ajax.php' + window.location.search,
		...
		function (data) {
			if (offset < data.count && offset < 1200) { // row limit
