...
if (isset($_POST['action']) && $_POST['action'] === 'iterate') {
	...
	$extra_field_mark = 'steam_img_fixed';
	...
	$description = str_replace('href="//koeln-webstudio.de/"', '', $description);

	preg_match('/.+steam\/([^\/]+)\/(\d+)\/header.+/', $description, $matches_found);

	$new_link = '//parser.gig-games.de/steam-images/'.@$matches_found[1].'-'.@$matches_found[2].'/header.jpg';

	$description = preg_replace('/\/\/cdn.+?\.jpg/', $new_link, $description);
	...
	if (isset($resp['Ack']) && $resp['Ack'] !== 'Failure') {
		arrayDB("UPDATE games SET $extra_field = '$extra_field_mark' WHERE id = '$games_id'");
	}
