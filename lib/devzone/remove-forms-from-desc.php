...
if (isset($_POST['action']) && $_POST['action'] === 'iterate') {
	...
	if($res[0]['extra_field2'] === 'form_removed'){
		echo json_encode([
			'offset' => $offset,
			'count' => $count,
			'res' => $res[0],
	...
	$description = preg_replace('/<form.+?<\/form>/s','<div class="search-form-was-here"></div>', $description);
	...
	if (isset($resp['Ack']) && $resp['Ack'] !== 'Failure') {
		arrayDB("UPDATE games SET extra_field2 = 'form_removed' WHERE id = '$games_id'");
	}
	//=============================================================================
...
function send_post(offset) {
	$.post('ajax.php' + window.location.search,
		...
		function (data) {
			if (offset < data.count && offset < 8150) {
