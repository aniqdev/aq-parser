...
if (isset($_POST['ebay_getprices'])) {
	...
	function saveResults($strJSON, $game_id) {
...
$multi_curl->success(function($instance) {
	...
	$game_id = $queryArr['tgame_id'];
    saveResults($instance->response, $game_id);
});
...
for ($j = $start; $j <= $start+4; $j++) {
	...
	$game = $games[$j-1]['name'];
	$url = Ebay_shopping::findItemsAdvanced_URL($game, 0, 1 , 1249);
	$multi_curl->addGet($url.'&tgame_id='.$games[$j-1]['id']);
}
$multi_curl->start(); // Blocks until all items in the queue have been processed.

for ($j = $start+5; $j <= $end; $j++) {
...
?>
	...
	<button id="ebay_getprices_multi" class="bttn">Спарсить цены</button>
