...
function getResultsFromApi($response, $blacklist, $blacksell){
	global $_ERRORS;
		$k = 0;
...
$_GET['multi_curl'] = new MultiCurl();

$_GET['multi_curl']->success(function($instance) {
	...
	//var_dump($query);
	//print_r($_GET['reqs']);
	// $req = $_GET['reqs'][md5($query)];
	// var_dump($req);

    $arrItem2 = getResultsFromApi($instance->response, $blacklist, $blacksell);
    //print_r($arrItem2);
    $arrItem2 = strictFilter($query, $arrItem2);
    //print_r($arrItem2);
    $_GET['arrItem1'] = array_merge($_GET['arrItem1'], $arrItem2);
    //print_r($_GET['arrItem1']);
});
$_GET['multi_curl']->error(function($instance) {
	global $_ERRORS;
...
function mainFunction(){
	...
	for ($j; $j <= $end; $j++) {
		...
		$item1_id    = 0;		$item1_name  = 'No results';
		$item1_price = 0;		$item1_desc  = 'No results';
		$item2_id    = 0;		$item2_name  = 'No results';
		$item2_price = 0;		$item2_desc  = 'No results';
		$item3_id    = 0;		$item3_name  = 'No results';
		$item3_price = 0;		$item3_desc  = 'No results';
    ...
    $arrItem = array();
    $_GET['arrItem1'] = array();
    $_GET['reqs'] = $idsArr = array();
    foreach ($requests as $req) {
        $reqEnc = urlencode($req);
        //$_GET['reqs'][md5($reqEnc)] = $req;
		$url = 'http://www.plati.ru/api/search.ashx?query='.$reqEnc.'&pagesize=500&response=json';

		//$result = file_get_contents($url);
		$_GET['multi_curl']->addGet($url);

    }
    $_GET['multi_curl']->start();
	$arrItem1 = $_GET['arrItem1'];
